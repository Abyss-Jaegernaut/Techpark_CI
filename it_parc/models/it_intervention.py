...
class ItIntervention(models.Model):
    ...
    _description = 'Intervention parc informatique'
    ...
    def create(self, vals_list):
        for vals in vals_list:
            equipment = self.env['it.equipement'].browse(vals.get('parc_equipment_id'))
            if equipment:
    ...
    def write(self, vals):
        if vals.get('parc_equipment_id'):
            equipment = self.env['it.equipement'].browse(vals['parc_equipment_id'])
            vals.setdefault('equipment_id', equipment.maintenance_equipment_id.id)
            vals.setdefault('company_id', equipment.company_id.id)
