...
class ItContrat(models.Model):
    ...
    name = fields.Char(string='Reference contrat', required=True, tracking=True)
    ...
    alert_ids = fields.One2many('it.alerte', 'contract_id', string='Alertes')
    notes = fields.Html(string='Notes')

    _sql_constraints = [
        ('name_unique', 'unique(name)', 'La reference contrat doit etre unique.'),
    ]
    ...
    def _check_contract_values(self):
        for contract in self:
            ...
            if contract.amount < 0:
                raise ValidationError(_("Le montant du contrat ne peut pas etre negatif."))
