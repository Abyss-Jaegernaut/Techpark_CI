...
class ItExportExcelWizard(models.TransientModel):
    _name = 'it.export.excel.wizard'
    _description = 'Assistant export Excel IT Parc'

    export_type = fields.Selection(
        [
            ('inventory', 'Inventaire complet'),
            ('maintenance_costs', 'Couts de maintenance mensuels'),
            ('expiring_contracts', 'Contrats expirant dans 60 jours'),
        ],
        string='Export',
        required=True,
        default='inventory',
    ...
    def action_export(self):
        ...
        filename = {
            'inventory': 'inventaire_it_parc.xlsx',
            'maintenance_costs': 'couts_maintenance_it_parc.xlsx',
            'expiring_contracts': 'contrats_expirants_it_parc.xlsx',
        }[self.export_type]
    ...
    def _formats(self, workbook):
        return {
            ...
            'warning': workbook.add_format({'border': 1, 'bg_color': '#FFF2CC'}),
            'critical': workbook.add_format({'border': 1, 'bg_color': '#F4CCCC'}),
        }
    ...
    def _build_inventory_sheet(self, workbook, formats):
        ...
        equipments = self.env['it.equipement'].search([], order='code, name')
    ...
    def _build_maintenance_costs_sheet(self, workbook, formats):
        ...
        headers = ['Equipement', 'Mois', 'Cout total', 'Nombre interventions']
        self._write_headers(sheet, headers, formats)
        grouped = defaultdict(lambda: {'cost': 0, 'count': 0})
        interventions = self.env['it.intervention'].search([('date_start', '!=', False)])
    ...
    def _build_expiring_contracts_sheet(self, workbook, formats):
        sheet = workbook.add_worksheet('Contrats 60 jours')
        headers = ['Reference', 'Fournisseur', 'Type', 'Fin validite', 'Jours restants', 'Montant', 'Etat']
        self._write_headers(sheet, headers, formats)
        contracts = self.env['it.contrat'].search([('days_left', '<=', 60), ('state', 'in', ('draft', 'active'))], order='date_end')
